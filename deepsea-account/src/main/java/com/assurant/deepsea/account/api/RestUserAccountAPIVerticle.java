package io.ensure.deepsea.account.api;
...
public class RestUserAccountAPIVerticle extends RestAPIVerticle {
  ...
  public void start(Future<Void> future) throws Exception {
    ...
    router.get(API_BUILD).handler(this::buildUser);

    String host = config().getString("user.account.http.address", "0.0.0.0");
    int port = config().getInteger("user.account.http.port", 8080);
  ...
  private void buildUser(RoutingContext context) {
	  accountService.initializePersistence(resultHandler(context, Json::encodePrettily));
  }
